...
revos_tar="${REVOS_ROOT}/ReVOS.tar"
revos_download="${DOWNLOADS}/ReVOS.tar"
pids=()
...
if [[ ! -s "${REVOS_ROOT}/.staged-complete" && ! -s "${revos_tar}" ]]; then
  download "https://huggingface.co/datasets/CIPLab-Video/decaf-rvos-revos/resolve/main/ReVOS.tar" "${revos_download}" & pids+=("$!")
fi
...
if [[ ! -s "${REVOS_ROOT}/.staged-complete" ]]; then
  if [[ ! -s "${revos_tar}" ]]; then mv "${revos_download}" "${revos_tar}"; fi
  # ReVOS is an uncompressed 47 GB tar containing many small JPEGs. Direct
  # extraction onto a parallel filesystem is metadata-bound. Index byte ranges
  # once, retain the archive, and extract only the metadata used by worklists.
  "${PYTHON_BIN}" -m concor_video.cli index-revos-tar \
    --source "${revos_tar}" --output "${revos_tar}.sqlite" --extract-root "${DATA_ROOT}"
  ...
  "${PYTHON_BIN}" -m concor_video.cli index-revos-masks \
    --source "${REVOS_ROOT}/mask_dict.json" --output "${REVOS_ROOT}/mask_dict.sqlite"
  touch "${REVOS_ROOT}/.staged-complete"
fi
