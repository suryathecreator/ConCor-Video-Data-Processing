...
New decisions use `selection_protocol: selected_instructions_v1`; each video stores `selected_sample_ids` and optional `allow_multiple`. The verified Parquet contains only confirmed instructions from accepted videos. Undecided/rejected videos and unconfirmed instructions are omitted. Existing decisions files without this protocol remain backward-compatible: their previously accepted videos without a selection still export all non-discarded instructions, while a newly selected video exports only its confirmed instruction(s). A hint in the UI makes this legacy behavior visible.
