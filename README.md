...
Open http://127.0.0.1:8000. For Ref-YT-VOS, the verifier suggests three tracklet-bearing instructions per video by default (change the count at the top), and verified export samples exactly one accepted instruction per accepted video. For ReVOS, a dedicated preview instead contains every nonexistent-object description plus one stable explicit and one stable implicit description for that video. ReVOS preview instructions start accepted. The top `1`/`2` controls (and keyboard keys when enabled) accept or reject the active instruction and advance to the next preview item, then the next video. All remaining ReVOS instructions stay visible below; **Add to preview + accept** includes one as a persistent override. Every accepted base or override preview instruction enters export, while rejected and unselected instructions do not. Each card shows its complete instruction with all exact linked spans. Click a colored span (or its link chip) to isolate its corresponding mask(s) in the video; **Preview all masks** restores the full instruction overlay. Existing `decisions.json` files remain compatible and retain their edits and video decisions; the ReVOS adapter adds `review_mode`, `preview_sample_ids`, `preview_override_sample_ids`, per-instruction `status`, and `accepted_sample_ids` when saved. Decisions checkpoint in browser storage and, at a selectable interval, to `--decisions`; export saves and downloads both the decisions and updated Parquet without overwriting the source.
