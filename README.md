...
Open http://127.0.0.1:8000. The verifier suggests three tracklet-bearing instructions per video by default (change the count at the top). Each suggestion and override shows its complete instruction with all exact linked spans. Click a colored span (or its link chip) to isolate its corresponding mask(s) in the video; **Preview all masks** restores the full instruction overlay. Click **Confirm to annotate** to open the full editor. Turn on **Select multiple instructions / target masks for this video** when useful. Each chosen instruction may itself link a text span to several tracklets. Verified export samples exactly one accepted instruction per accepted video; other accepted choices and all edits stay in `decisions.json` for later. Older decisions files are supported: one previously accepted, non-discarded instruction is sampled, rather than exporting them all. You can edit language and exact links, delete spans/tracklets, play or step through frames, and accept/reject by video. Decisions checkpoint in browser storage and, at a selectable interval, to `--decisions`; export saves and downloads both the decisions and updated Parquet without overwriting the source.
