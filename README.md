...
Open http://127.0.0.1:8000. The verifier suggests three tracklet-bearing instructions per video by default (change the count at the top). Preview one, then click **Confirm to annotate**; the full editor opens for that instruction. Every remaining instruction stays in the lower override list. Turn on **Select multiple instructions / target masks for this video** when useful. Each chosen instruction may itself link a text span to several tracklets. New verified exports include only confirmed instructions from accepted videos; an unchosen instruction is not silently included. You can edit language and exact links, delete spans/tracklets, play or step through frames, and accept/reject by video. Decisions checkpoint in browser storage and, at a selectable interval, to `--decisions`; export saves and downloads both the decisions and updated Parquet without overwriting the source. Older `decisions.json` files without selection fields retain their previous all-instructions export behavior until a video is explicitly reselected.
