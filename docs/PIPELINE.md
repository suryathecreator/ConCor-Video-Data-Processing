...
SIGUSR1 requests a drain: finish the active instruction, fsync its JSON, close the session, then requeue. Records, errors, and worklists live outside node-local storage. Reproducible decoded-frame scratch and mutable Torch/Triton/CUDA caches are job/task/restart-specific and removed on exit. Immutable checkpoint/runtime staging is node-shared under `flock`. Before staging, workers remove abandoned atomic partial copies under the same locks. If node scratch has less than 5 GiB free, a worker skips the local checkpoint copy, uses an isolated campaign-side mutable cache, and loads a lock-protected immutable Python runtime from campaign storage. This prevents a full `/tmp` or a minimal driver environment from corrupting or indefinitely failing a shard. CUDA allocation uses expandable segments, and an OOM forces session garbage collection plus allocator-cache release before a bounded checkpoint retry. The retry marks its whole video session memory-safe, offloads video/state tensors to CPU, reduces SAM3.1 grounding and postprocessing from 16-frame batches to one frame, and caps crowded prompt tracking at 16 objects; normal samples retain the faster GPU-resident batch-16, 32-object path. A remaining OOM receives one final 8-object attempt. SAM31_MEMORY_SAFE_GROUNDING_BATCH_SIZE and SAM31_MEMORY_SAFE_MAX_OBJECTS can tune the safe limits for different hardware. An OOM that has already used the one-frame, 8-object path remains a terminal audited error.
