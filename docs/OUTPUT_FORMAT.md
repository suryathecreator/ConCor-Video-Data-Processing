...
The verifier groups rows by dataset, split, and video_id. New review decisions store a deterministic-suggestion count globally and explicit `selected_sample_ids` per video. Exactly one eligible accepted instruction per accepted video enters verified export; undecided/rejected videos and discarded instructions are omitted. The per-video `allow_multiple` flag permits several confirmed instructions, all of which remain in `selected_sample_ids` and `accepted_sample_ids` while `sampled_sample_id` records the one exported. For older video-level decisions, all non-discarded instructions are treated as accepted candidates; one is deterministically sampled, and the rest are archived in `accepted_sample_ids` when decisions are saved. Deleted tracklets are removed, invalid/deleted links are removed, and edited exact spans rebuild span_links. Source Parquet is never overwritten.
